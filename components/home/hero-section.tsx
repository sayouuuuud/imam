          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
            <div className="flex items-center justify-center gap-3 text-center">
              ...
              {heroData.notice_link ? (
                ...
                <span className="text-base font-medium text-foreground">{heroData.notice_text}</span>
              )}
            </div>
          </div>
        </div>
                ...
                {bookImageUrl ? (
                  <div className="relative w-[280px] sm:w-[320px] h-[400px] sm:h-[460px] rounded-xl overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-foreground/90 to-transparent p-4">
                      <p className="text-background font-bold text-lg">{featuredBook?.title}</p>
                      {featuredBook?.author && <p className="text-background/80 text-sm">{featuredBook.author}</p>}
                    </div>
                  </div>
                ) : (
