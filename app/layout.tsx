...
export default async function RootLayout({
  ...
  // Types this layout already emits itself via generateWebsiteSchema() /
  // generatePersonSchema(). Re-emitting them from admin JSON creates two
  // competing entities for the same thing: the page shipped one WebSite named
  // "السيد مراد سلامة" and another named "الشيخ السيد مراد سلامة", so Google
  // could not decide which was authoritative and displayed the bare domain
  // instead of either name. Drop duplicates and keep the canonical ones.
  const RESERVED_SCHEMA_TYPES = new Set(["WebSite", "Person"])

  let customStructuredData: unknown = null
  if (siteSettings.structured_data && siteSettings.structured_data.trim()) {
    try {
      const parsed = JSON.parse(siteSettings.structured_data)
      const entries = (Array.isArray(parsed) ? parsed : [parsed]).filter((entry) => {
        const type = entry?.["@type"]
        const types = Array.isArray(type) ? type : [type]
        const conflicts = types.some((t: unknown) => typeof t === "string" && RESERVED_SCHEMA_TYPES.has(t))
        if (conflicts) {
          console.warn(
            `[seo] Ignoring "${types.join(", ")}" from site_settings.structured_data — ` +
              "this type is already generated by the app and a duplicate would conflict.",
          )
        }
        return !conflicts
      })
      if (entries.length > 0) {
        customStructuredData = entries.length === 1 ? entries[0] : entries
      }
    } catch (e) {
      // Malformed JSON — log and skip instead of crashing the whole page.
      console.warn("[seo] Invalid structured_data JSON in site_settings:", e)
    }
  }
