...
function getEventTypeStyles(type: string) {
  switch (type) {
    ...
    case "seerah":
      return { bg: "bg-orange-50", border: "border-orange-500", text: "text-orange-600" }
    case "friday":
      return { bg: "bg-primary/10", border: "border-primary", text: "text-primary" }
    case "aqeedah":
      return { bg: "bg-purple-50", border: "border-purple-500", text: "text-purple-600" }
    default:
...
// Get Arabic day name from English
function getDayNameArabic(day: string | null): string {
  if (!day) return ""
...
export default async function SchedulePage({
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dayEvents = (oneTimeEvents || []).filter((e) => e.event_date === dateStr)
 ...
 {/* Header */} <header className="bg-slate-900 py-12 relative overflow-hidden">
...
<span className="inline-block px-3 py-1 bg-slate-700 text-secondary text-xs rounded-full mb-4 border border-secondary/20">
 تنظيم الوقت </span>
...
<div className="container mx-auto px-4 py-12">
 {weeklyEvents && weeklyEvents.length > 0 && ( <div className="mb-12 bg-slate-800 rounded-2xl p-6 border border-border">
<h2 className="text-xl font-bold text-foreground mb-6 flex items-center gap-2">
 <CalendarRange className="h-5 w-5 text-primary" />
 الجدول الأسبوعي الثابت </h2>
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
 {weeklyEvents.map((item: ScheduleEvent) => ( <div key={item.id}
className="bg-slate-700 rounded-xl p-4 border border-border">
<div className="flex items-center gap-2 mb-2">
 <span className="text-xs bg-primary/10 text-primary px-2 py-1 rounded-full">
 {getDayNameArabic(item.day_of_week)} </span>
<span className="text-xs text-text-muted">
{formatTime12h(item.event_time)}</span>

 {item.is_live && ( <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full flex items-center gap-1">
...
{item.title}</h3>
 ...
 {item.location && ( <p className="text-xs text-text-muted mt-1 flex items-center gap-1">
...
<div className="flex items-center bg-slate-800 rounded-xl shadow-sm p-2 border border-border">
...
<CalendarDays className="h-4 w-4" />
 شهر </button>
<button className="px-4 py-2 bg-slate-800 text-text-muted border border-border hover:bg-slate-700 rounded-lg text-sm transition flex items-center gap-2">
 ...
 {/* Calendar Grid */} <div className="bg-slate-800 rounded-2xl shadow-sm overflow-hidden border border-border">
 {/* Day Headers */} <div className="grid grid-cols-7 border-b border-border bg-muted/50">
 {[0, 1, 2, 3, 4, 5, 6].map((dayIndex) => ( <div key={dayIndex}
className={`py-4 text-center text-sm font-bold ${dayIndex === 6 ? "text-green-700" : "text-text-muted"}`} >
...
className={`min-h-[140px] p-2 relative group transition ${ !calDay.isCurrentMonth ? "bg-slate-800 text-text-muted/30" : calDay.isFriday ? "bg-green-50/50 hover:bg-green-100" : "bg-slate-800 hover:bg-slate-700" }`} >
<span className={`text-sm ${ calDay.isFriday && calDay.isCurrentMonth ? "font-bold text-green-700" : calDay.isCurrentMonth ? "font-medium text-foreground" : "" }`} >
 ...
 {calDay.events.map((event) => { const styles = getEventTypeStyles(event.event_type) 
...
<MapPin className="h-2.5 w-2.5" />
 ...
 {/* Legend */} <div className="mt-8 flex flex-wrap justify-center gap-6">
