...
export default function BookDetailPage({ params }: PageProps) {
  ...
  const getCoverImageUrl = (book: any) => {
    console.log('📖 [BOOK DETAIL] getCoverImageUrl Input:', {
      bookId: book.id,
    ...
    // Fallback to cover_image or empty
    const finalUrl = book.cover_image_path || book.cover_image || ""
    console.log('📄 Final URL (fallback):', finalUrl)
    return finalUrl
  }
  ...
  // Clear console and debug logging for book detail
  console.clear()
  console.log('🧹 Console cleared for fresh debugging')
  console.log('📖 Book Detail Debug:', {
    bookId: book.id,
    title: book.title,
    raw_cover_image_path: book.cover_image_path,
    raw_cover_image: book.cover_image,
    processed_coverImageUrl: coverImageUrl,
    hasCoverPath: !!book.cover_image_path,
    pathStartsWithUploads: book.cover_image_path?.startsWith('uploads/'),
    pathStartsWithHttp: book.cover_image_path?.startsWith('http'),
    pathContainsApiDownload: book.cover_image_path?.includes('/api/download'),
    pathContainsB2Url: book.cover_image_path?.includes('backblazeb2.com'),
